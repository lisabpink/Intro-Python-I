...
[x-reviewed]_ `hello` -- Hello world
[x-reviewed]_ `bignum` -- Print some big numbers
[x-reviewed]_ `datatypes` -- Experiment with type conversion
[x-reviewed]_ `modules` -- Learn to import from modules
[x-reviewed]_ `printing` -- Formatted print output
[x-reviewed]_ `lists` -- Python's version of arrays
[x-reviewed]_ `tuples` -- Immutable lists typically for heterogenous data
[x-reviewed]_ `slices` -- Accessing parts of lists
[x-reviewed]_ `comprehensions` -- List comprehensions
[x-reviewed]_ `dictionaries` -- Dictionaries
[x-reviewed]_ `functions` -- Functions
[x-reviewed]_ `args` -- Arguments and Keyword Arguments
[x-reviewed]_ `scopes` -- Global, Local, and Non-Local scope
[x-reviewed]_ `file_io` -- Read and write from files
[x-reviewed]_ `cal` -- Experiment with module imports and implement a text-based calendar
[x-reviewed]_ `classes` -- Classes and objects
